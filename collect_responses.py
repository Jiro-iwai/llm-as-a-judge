...
def initialize_processing_time_log(log_file: str) -> None:
    ...
    try:
        ...
        log_warning(
            f"処理時間ログファイルを初期化できませんでした: {exc}. ログ出力をスキップします。"
        )
...
def log_processing_time_entry(
    ...
    question_part = f" 質問{question_number}" if question_number is not None else ""
    line = f"📥 [{model_name}] {label}{question_part} 経過時間: {elapsed_time:.2f}秒"

    try:
        with open(log_file, "a", encoding="utf-8") as file:
            file.write(line + os.linesep)
    except OSError as exc:
        log_warning(f"処理時間ログへの書き込みに失敗しました ({log_file}): {exc}")
...
def generate_processing_time_reports(
    ...
    if not log_file:
        log_warning(
            "処理時間ログファイルが指定されていないため、可視化をスキップします。"
        )
        return
    ...
    if not log_path.exists():
        log_warning(
            f"処理時間ログファイル '{log_file}' が見つかりません。可視化をスキップします。"
        )
        return
    ...
    try:
        ...
        log_warning(f"処理時間ログの解析に失敗しました: {exc}")
        return

    if not question_numbers:
        log_warning("処理時間ログに有効なデータがありません。可視化をスキップします。")
        return
    ...
    try:
        ...
        log_success("処理時間比較レポートを生成しました。")
        log_info(f"⏱️ ログファイル: {log_file}", indent=1)
        log_info(f"🖼  比較チャート: {comparison_path}", indent=1)
        log_info(f"🖼  統計チャート: {statistics_path}", indent=1)
        log_info(f"📝 サマリー: {summary_path}", indent=1)
    except Exception as exc:
        log_warning(f"処理時間比較レポートの生成に失敗しました: {exc}")
