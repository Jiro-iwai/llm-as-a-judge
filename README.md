...
このリポジトリにはGitHub ActionsによるCIパイプラインが設定されています。

- **トリガー**: `main`ブランチへの`push`および`pull_request`
- **実行内容**: `make test`を実行し、フォーマット、リンター、型チェック、スクリプトテスト、ユニットテストをすべて実行します
- **環境**: Ubuntu latest、Python 3.13、`uv`を使用した依存関係管理
- **キャッシュ**: `uv`の依存関係キャッシュを利用してCI時間を短縮

PRを作成すると、自動的にCIが実行され、すべてのテストがパスすることを確認できます。
